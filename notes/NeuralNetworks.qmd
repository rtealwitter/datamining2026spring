...
The function $v : \mathbb{R}^{d} \to \mathbb{R}$ maps the input values $u_1, \ldots, u_d$ to an output, which itself is passed to another function.
...
By the multivariate chain rule, the partial derivative of the loss function with respect to $u$ is given by
$$
...
By the time we reach $u$ in the backward pass, the first set of partial derivatives is already known from the prior layer, and the second set of partial derivatives only depend on how $u$ is used in the functions $v_1, \ldots, v_n$.
...
Originally intended for matrix multiplication in graphics rendering, the development of Graphics Processing Units (GPUs) allowed us to efficiently implement backpropagation.

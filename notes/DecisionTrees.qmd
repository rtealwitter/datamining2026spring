...
$$
There are several ways to define the loss of a leaf, including:
...
**Entropy**:
Like in logistic regression, we can use the entropy of the leaf to measure the error.
That is,
$$
\mathcal{L}_\text{Entropy}(\ell) = H(\ell) = -\sum_{c} p^{(\ell)}_c \log(p^{(\ell)}_c)
$$
...
Then, partitioning the summation, we can write the loss as
$$
\begin{align}
\mathcal{L}(F_t) &=
...
Each weak learner can be trained quickly, and the resulting ensemble can be understood as a weighted vote of the individual trees.
...
$$
In practice, finding a function that exactly matches the negative gradient is difficult, so we will instead find a function that approximates it. That is, we train a weak learner $f_t$ to fit the points
$$
(\mathbf{x}^{(i)}, -\frac{ \partial \mathcal{L}(y^{(i)}, F_{t-1}(\mathbf{x}^{(i)}))}{\partial F_{t-1}(\mathbf{x}^{(i)})})
$$ for $i \in \{1, \ldots n\}$.
When the loss is the squared error loss, i.e.,
$$
\mathcal{L}(y, F_{t-1}(\mathbf{x})) = \frac12 (F_{t-1}(\mathbf{x}) - y)^2,
$$
the negative gradient of the loss with respect to the prediction is exactly the residual between the true label and the prior prediction, i.e.,
$$
-\frac{ \partial \mathcal{L}(y, F_{t-1}(\mathbf{x}))}{\partial F_{t-1}(\mathbf{x})} = y - F_{t-1}(\mathbf{x}).
$$
In this way, gradient boosting has a nice interpretation as an iterative process of fitting a model to the residuals of the prior model.

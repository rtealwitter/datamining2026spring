...
Let $\mathbf{W}_0 \in \mathbb{R}^{d \times k}$ be the encoder weight matrix, and $\mathbf{W}_1 \in \mathbb{R}^{k \times d}$ be the decoder weight matrix.
...
Without loss of generality, suppose that $n \geq d$ (if not, we can just work with $\mathbf{X}^\top$ instead of $\mathbf{X}$).
We can write
$$
\mathbf{X} = \mathbf{U} \mathbf{\Sigma} \mathbf{V}^\top
$$
where $\mathbf{U} \in \mathbb{R}^{n \times d}$ and $\mathbf{V} \in \mathbb{R}^{d \times d}$ have orthonormal columns, and $\mathbf{\Sigma} \in \mathbb{R}^{d \times d}$ is a diagonal matrix with non-negative entries.
...
$$
where $\sigma_1 \geq \sigma_2 \geq \ldots \geq \sigma_d \geq 0$ are the singular values of $\mathbf{X}$, and $\mathbf{u}_i \in \mathbb{R}^{n \times 1}$ and $\mathbf{v}_i \in \mathbb{R}^{d \times 1}$ are the left and right singular vectors, respectively.
The rank of $\mathbf{X}$ is $r$, which is the number of non-zero singular values.
...
For a square matrix $\mathbf{M}$, the trace $\text{tr}(\mathbf{M})$ is the sum of the diagonal entries, and it has the following properties:

* **Frobenius norm**: $\| \mathbf{X} \|_\text{F}^2 = \text{tr}(\mathbf{X}^\top \mathbf{X})$ for any $\mathbf{X} \in \mathbb{R}^{n \times d}$.
...
* **Cyclic property**: $\text{tr}(\mathbf{A} \mathbf{B}) = \text{tr}(\mathbf{B} \mathbf{A})$
...
Since $\tilde{\mathbf{X}}$ is a rank-$k$ matrix, we can write $\tilde{\mathbf{X}} = \mathbf{X} \mathbf{W W^\top}$ for some $\mathbf{W} \in \mathbb{R}^{d \times k}$ where $\mathbf{W}^\top \mathbf{W} = \mathbf{I}$.
...
= \sum_{i=1}^d \mathbf{u}_i \sigma_i  \mathbf{v}_i^\top \sum_{j=1}^k \mathbf{v}_j \mathbf{v}_j^\top
...
So if we have columns that are linearly dependent, then the error tends to be lower.
...
If the original data is close to the reconstructed data, then $\| \tilde{\mathbf{x}} \|_2 \approx \| \mathbf{x} \|_2$, and the magnitude of the latent representation $\| \mathbf{z} \|_2$ will be identical to the reconstruction.
...
Consider the columns of $\mathbf{V}^\top$, denoted by $\mathbf{y}_i$ and $\mathbf{y}_j$ for words $i$ and $j$.
If the reconstruction $\tilde{\mathbf{X}}$ is close to the original $\mathbf{X}$, then $\langle \mathbf{z}, \mathbf{y}_i \rangle \approx 1$ if word $i$ appears in the document, and $\langle \mathbf{z}, \mathbf{y}_j \rangle \approx 1$ if word $j$ appears.
In this case, $\langle \mathbf{y}_i, \mathbf{y}_j \rangle$ will likely be large.
...
In methods related to word2vec, such as PMI matrix factorization, the similarity is given by
$$

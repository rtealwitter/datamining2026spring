...
In terms of the matrix multiplication view of linear regression, we can think of adding additional columns to the feature matrix $\mathbf{X}$ by multiplying existing columns together, or raising them to a power.
...
**Claim:** The training fit of the regression model can only be improved by adding additional features. Formally,
...
An alternative is to use *cross-validation*, where we split the non-test data into $k$ folds, and use each fold as the validation set once while training on the remaining $k-1$ folds.
...
So what model achieves the lowest generalization error?
